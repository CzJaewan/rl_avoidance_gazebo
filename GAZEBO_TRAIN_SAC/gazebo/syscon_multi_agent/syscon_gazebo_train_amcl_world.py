...
class StageWorld():
    ...
    def generate_goal_point_gazebo_4agent(self):
        [x_g, y_g] = self.generate_random_goal_4agent()

        self.visual_gazebo_goal_point(x_g, y_g)
    ...
    def set_gazebo_pose_4agent(self):
        
        self.control_vel([0, 0])

        print("set_pose")
        [x, y, w] = self.generate_random_pose_4agent()

        state_msg = ModelState()
        state_msg.model_name = 'robot_' + str(self.index)
        state_msg.pose.position.x = x
        state_msg.pose.position.y = y
        state_msg.pose.position.z = 0
    ...
    def generate_random_pose_4agent(self):
    ...
    def generate_random_goal_4agent(self):
        [x_robot, y_robot, theta] = self.get_self_stateGT()

        rand_area = np.random.randint(0,2)
    ...
    def visual_gazebo_goal_point(self, x, y):
        
        state_msg = ModelState()
        state_msg.model_name = 'goal_box_' + str(self.index)
        state_msg.pose.position.x = x
        state_msg.pose.position.y = y
        state_msg.pose.position.z = 0

        qtn = tf.transformations.quaternion_from_euler(0, 0, 0, 'rxyz')
