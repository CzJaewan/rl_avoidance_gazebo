...
class StageWorld():
    def __init__(self, beam_num, index, num_env, robot_radius):
        ...
        object_state_topic = 'robot_' + str(index) + '/odom'
	...
	# -----------Look A head --------------
        ...
        lah_topic = 'robot_' + str(index) + '/lookAhead_point'
        self.lah_sub = rospy.Subscriber(lah_topic, PoseStamped, self.lah_callback)


        # -----------test--------------------
        waypoint_pose_topic = 'robot_' + str(index) + '/waypoint_pose'
        ...
        self.model_name = 'robot_' + str(index)
